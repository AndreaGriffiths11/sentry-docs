...
enum class InstrumentationFeature {
    /**
     * When enabled and sentry-android-sqlite is on the classpath, bytecode wraps
     * SupportSQLiteOpenHelper.Factory and instruments Room DAOs (db.sql.query and db.sql.room).
     * Does not instrument SQLiteDriver or Room.setDriver; use SentrySQLiteDriver manually.
     *
...
For `InstrumentationFeature.DATABASE`, the plugin instruments `SupportSQLiteOpenHelper.Factory` and Room DAOs when `sentry-android-sqlite` is present. It does not wrap `androidx.sqlite.SQLiteDriver` or call `Room.databaseBuilder.setDriver`. Apps on the [SQLite driver path](/platforms/android/integrations/room-and-sqlite/#sqlite-driver-room-27) must use `SentrySQLiteDriver.create(...)` manually. See [Room and SQLite](/platforms/android/integrations/room-and-sqlite/#choosing-an-instrumentation-path) for avoiding duplicate `db.sql.query` spans.
...
To test unreleased versions of the Sentry Android Gradle Plugin, see <PlatformLink to="/configuration/pre-release-versions">Pre-Release Versions</PlatformLink>.
