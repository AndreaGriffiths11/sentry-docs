...
Custom instrumentation (`SentrySupportSQLiteOpenHelper` and `SentrySQLiteDriver`) is supported in `sentry-android-sqlite`, SDK version `6.21.0` and above.
...
| **SQLite driver (Room 2.7+)** | `Room.databaseBuilder(...).setDriver(...)` | **Manual:** wrap the driver with `SentrySQLiteDriver.create(...)` (SDK ≥ 6.21.0). SAGP does **not** instrument `SQLiteDriver`. | SAGP when DATABASE tracing is enabled |
...
Supported in `sentry-android-sqlite`, SDK version **6.21.0** and above (same module as `SentrySupportSQLiteOpenHelper`).
